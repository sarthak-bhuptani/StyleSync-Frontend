...
export const BudgetPage = () => {
  ...
  const spentThisMonth = budgetData.spentThisMonth !== undefined
    ? budgetData.spentThisMonth
    : purchases.reduce((acc, p) => acc + (Number(p.price) || 0), 0);
  ...
  const handleAddPurchase = async (e) => {
    ...
    const itemPrice = Number(newPurchase.price) || 1999;
    const itemCategory = newPurchase.category || 'Clothing';
    ...
    setBudgetData(prev => {
      const updatedSpent = (prev.spentThisMonth || 0) + itemPrice;
      const updatedCategories = (prev.categories || categories).map(cat => {
        if (cat.name.toLowerCase().includes(itemCategory.toLowerCase()) || itemCategory.toLowerCase().includes(cat.name.toLowerCase())) {
          return { ...cat, spent: (cat.spent || 0) + itemPrice };
        }
        return cat;
      });
      const updated = {
        ...prev,
        spentThisMonth: updatedSpent,
  ...
  const handleDeletePurchase = async () => {
    ...
    try {
      ...
      setBudgetData(prev => {
        const newSpent = Math.max(0, (prev.spentThisMonth || 0) - purchasePrice);
        const updatedCategories = (prev.categories || categories).map(cat => {
          if (purchaseCategory && (cat.name.toLowerCase().includes(purchaseCategory.toLowerCase()) || purchaseCategory.toLowerCase().includes(cat.name.toLowerCase()))) {
            return { ...cat, spent: Math.max(0, (cat.spent || 0) - purchasePrice) };
          }
          return cat;
        });
        const updatedBudget = {
        ...
        localStorage.setItem('stylesync_budget', JSON.stringify(updatedBudget));
        return updatedBudget;
      });

      showToast(`Removed "${purchase.productName || 'Item'}" and restored ₹${purchasePrice.toLocaleString()} to budget`, 'success');
    } catch (err) {
  ...
  const handleClearAllPurchases = async () => {
    ...
    try {
      for (const p of purchases) {
        await purchaseApi.deletePurchase(p.id || p._id);
      }
      setPurchases([]);
      setBudgetData(prev => {
        const updatedBudget = {
          ...prev,
          spentThisMonth: 0,
          categories: (prev.categories || categories).map(c => ({ ...c, spent: 0 }))
        };
        localStorage.setItem('stylesync_budget', JSON.stringify(updatedBudget));
        return updatedBudget;
      });
      showToast('Cleared all purchase history & restored full budget', 'success');
    } catch (err) {
      console.error('Failed to clear history:', err);
      showToast('Failed to clear purchase history', 'error');
    } finally {
      setIsDeleting(false);
  ...
  return (
    <div className="space-y-4 sm:space-y-5 animate-fade-in max-w-5xl mx-auto pb-16">
            ...
            <div className="flex items-baseline gap-2 mt-0.5">
              ...
              <span className={`text-[11px] font-bold px-2 py-0.5 rounded-md border ${
                percentSpent <= 70
                  ...
                  : 'text-rose-800 bg-rose-50 border-rose-200/60'
              }`}>
                {percentSpent <= 70 ? 'On Track' : percentSpent <= 95 ? 'Near Cap' : 'Over Limit'}
              </span>
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              {purchases.length > 0 && (
                <button
                  ...
                  className="text-[11px] font-bold text-rose-600 hover:text-rose-700 bg-rose-50 hover:bg-rose-100 px-2 py-0.5 rounded-lg border border-rose-200 transition-colors cursor-pointer"
                >
                  Clear All
                </button>
              )}
                ...
                >
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 border border-transparent hover:border-rose-200 transition-all cursor-pointer ml-0.5"
                      title="Delete purchase from ledger"
                    >
      ...
      {/* Delete Single Purchase Confirmation Modal */}
      <Modal
      ...
      >
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-3.5 rounded-2xl bg-rose-50 border border-rose-100">
            ...
            <div>
              <h4 className="text-xs font-bold text-rose-950">Remove from Ledger?</h4>
              <p className="text-[11px] text-rose-700 mt-0.5">
                This item will be deleted and ₹{Number(deleteModal.purchase?.price || 0).toLocaleString()} will be restored to your remaining budget.
              </p>
            </div>
          </div>
          ...
          <div className="p-3 rounded-2xl bg-stone-50 border border-stone-200/80 flex items-center gap-3">
            <img
              src={deleteModal.purchase?.image || 'https://images.unsplash.com/photo-1596755094514-f87e34085b2c?auto=format&fit=crop&w=200&q=80'}
              alt="Item"
              className="w-10 h-10 rounded-xl object-cover bg-white border border-stone-200 shrink-0"
            />
            <div className="min-w-0 flex-1">
              <p className="text-xs font-bold text-slate-900 truncate">{deleteModal.purchase?.productName}</p>
              <p className="text-[11px] text-stone-500 font-semibold">
                ₹{Number(deleteModal.purchase?.price || 0).toLocaleString()} · {deleteModal.purchase?.category}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-2 pt-2 border-t border-stone-100">
            <button
              ...
              className="px-4 py-2 rounded-xl border border-stone-200 text-stone-700 hover:bg-stone-50 text-xs font-bold transition-all cursor-pointer disabled:opacity-50"
            >
              Cancel
            </button>
            <button
      ...
      >
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-3.5 rounded-2xl bg-rose-50 border border-rose-100">
            ...
            <div>
              <h4 className="text-xs font-bold text-rose-950">Reset Entire Ledger?</h4>
              <p className="text-[11px] text-rose-700 mt-0.5">
                All {purchases.length} recorded purchases will be permanently cleared and your monthly spend will reset to ₹0.
              </p>
            </div>
          </div>
