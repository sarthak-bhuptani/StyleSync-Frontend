      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
            {wardrobe.length} verified pieces calibrated in your AI styling capsule.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="px-4 sm:px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs sm:text-sm font-bold rounded-xl shadow-subtle hover:shadow transition-all flex items-center gap-2 active:scale-95"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {filteredItems.map((item) => (
            <WardrobeCard
              ...
              onDelete={(id) => deleteWardrobeItem(id)}
            />
          ))}
        </div>
      ) : (
        <EmptyState
          ...
          description={
            searchQuery
              ...
              : 'Add your first item so StyleSync can provide accurate neural recommendations.'
          }
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="flex justify-between py-1.5">
                <span className="text-slate-500">Cost per Wear:</span>
                <strong className="text-slate-900 font-bold">
                  ₹{Math.round((selectedItemForView.price || 3000) / Math.max(1, selectedItemForView.usageCount || 1))} / wear
                </strong>
              </div>
            </div>
            <button
